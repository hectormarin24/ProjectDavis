...
export default class bathroomSort extends Phaser.Scene {
    ...
    create() {
        ...
        // Toilet (made a bit bigger)
        this.bowl = this.add
            .image(cx, this.scale.height - 120, 'toilet')
            .setOrigin(0.5)
            .setScale(1.3); // was 1.1
        ...
        if (gs.slowMode) spawnDelay *= 1.15;
        ...
        // Difficulty ramp (still respects "one item at a time")
        this.time.addEvent({
            delay: 10000,
            callback: () => {
    ...
    spawnFallingItem() {
        ...
        // FIX: Toilet paper only drops in the middle of the screen
        let x;
        if (isGood) {
            // TP spawns in a narrow band around center
            x = Phaser.Math.Between(cx - 60, cx + 60);
        } else {
            // Other items can spawn across the width
        ...
        if (gs.slowMode) fallDuration *= 1.25;
        ...
        this.tweens.add({
            ...
            onUpdate: () => {
                if (
                    !sprite.hasEnded &&
                    Phaser.Geom.Rectangle.Contains(this.bowlZone, sprite.x, sprite.y)
                ) {
